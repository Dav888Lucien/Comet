...
def get_product_info(types, soup):
    if types == "description":
        ...
        tags = soup.find('div', class_ = "col-xs-6 col-md-7 col-lg-8").descendants
    else:
        return "Wrong String!"
...
def get_category(soup):
    tag = soup.find('input', id = "productCategory")
...
def get_price(soup):
    tag = soup.find('span', class_ = "value canada-currency-size")
    return tag.text
...
def extract_and_load_all_data(soup):
    ...
    output_data = open('OutputData.csv', 'a', encoding = 'utf-8', newline="")
    ...
    item_list = [a for a in collected_data.keys()]
   ...
   # csv_writer.writerow(i)
    ...
    content_list = [b for b in collected_data.values()]
